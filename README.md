...
- [9. 2026-08-22 更新：SSH 加固 + systemd 常驻验收](#9-2026-08-22-更新ssh-加固--systemd-常驻验收)
...
| 11 | SSE 频繁断连（RikkaHub 连不上） | 换 Streamable HTTP（8002 + FastMCP + uvicorn），详见[第 7 章](#7-2026-08-11-更新vps失联--sse根治) |
...
## 9. 2026-08-22 更新：SSH 加固 + systemd 常驻验收

8 月 22 日实录：发现 SSH 密码尝试后，完成 Fail2Ban 配置与防火墙链实际验证；从新终端验证私钥登录，确认密码认证关闭；最后把 Python 服务交给 systemd 常驻，并通过完整日志验证时区、安静时段与循环持久化。公开文档中的端口、用户与路径均已使用占位符。

📄 完整记录见：[docs/2026-08-22-ssh-fail2ban-and-systemd.md](docs/2026-08-22-ssh-fail2ban-and-systemd.md)

**一句话总结**：
1. **先验证密钥，再禁密码**，始终保留旧会话作为退路
2. **Fail2Ban 不能只看 running**，还要验证 jail、过滤器与防火墙链
3. **systemd 常驻不能只看进程**，还要从业务日志确认真实行为
